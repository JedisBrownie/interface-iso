...
export default function Brouillon(){
    ...
    [
        ...
        {idMatricule : 4726 , nom : 'Tiavina Rakototafika' , listeDocument : [
            {id:1,referenceDocument:'FI3100-20240418-1',nomStatus:'Brouillon',status:'Redaaction',dateCreation: '21/07/2024',idTypeDocument: 2 ,nomTypeDocument : 'Sous Processus',nom:"Implication securite routiere",nombreRevision:4,listeProcessusLie:[{idProcessusLie:3100}],modification:false},
        ]},
        {idMatricule : 1269 , nom : 'Narindra Andrianantenaina' , listeDocument : [
        ]},
        {idMatricule : 1258 , nom : 'Vero Andriamalala' , listeDocument : [
        ]},
        {idMatricule : 4583 , nom : 'Andry Rasolofomanana' , listeDocument : [
        ]},
        {idMatricule : 5246 , nom : 'Miantsa Razafindrakoto' , listeDocument : [
        ]}
    ];

    const userToken = localStorage.getItem("userToken");

    const getFilteredData = (userToken, data) => {
        if (!userToken || !data) {
            console.error("UserToken ou données manquantes");
            return [];
        }
    
        const { matricule, commite_direction } = JSON.parse(userToken);
    
        // Si l'utilisateur est membre du comité de direction, retourne tout
        if (commite_direction === true) {
            return data;
        }
    
       
        return data.filter(item => item.idMatricule === parseInt(matricule, 10));
    };


    useEffect(() => {
        const filtered = getFilteredData(userToken, data);
    ...
    return(
        <>
            ...
            <div className="ui container" style={{paddingTop:'8.5em'}}>
                <AccordionListBrouillon data = {filteredData}></AccordionListBrouillon>
